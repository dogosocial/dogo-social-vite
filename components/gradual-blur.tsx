...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const config = useMemo(() => {
    ...
    return mergeConfigs(DEFAULT_CONFIG, {
      preset,
      position,
      strength,
      height,
  ...
  const blurDivs = useMemo(() => {
    ...
    const currentStrength =
      isHovered && config.hoverIntensityProp ? config.strength * config.hoverIntensityProp : config.strength
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      ...
      pointerEvents: config.hoverIntensityProp ? "auto" : "none",
  ...
  useEffect(() => {
    if (isVisible && config.animated === "scroll" && config.onAnimationCompleteProp) {
      const t = setTimeout(() => config.onAnimationCompleteProp(), Number.parseFloat(config.duration) * 1000)
      return () => clearTimeout(t)
    }
  }, [isVisible, config.animated, config.onAnimationCompleteProp, config.duration])
  ...
  return (
    <div
      ...
      onMouseEnter={config.hoverIntensityProp ? () => setIsHovered(true) : undefined}
      onMouseLeave={config.hoverIntensityProp ? () => setIsHovered(false) : undefined}
    >
